...
9. System-font consistency remains a possible polish item. Do not add font files unless the team chooses a specific permissively licensed font and updates `LICENSES.md`.
...
- `app/globals.css`: styles the target-date notice, table focus state, and mobile scroll cue.
...
9. Commit only after the checks pass. Use a focused message such as `fix: harden fixture imports and target advice`.
...
The app was already functionally strong. The implementation work is intentionally narrow: reject bad fixture boundaries, prevent misleading target-date output, make mobile evidence discoverable, shorten the demo script, and lock those behaviors with tests. If the next agent must stop early, the remaining work is verification, not a redesign.
